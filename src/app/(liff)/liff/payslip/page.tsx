...
export default async function LiffPayslipPage({
    ...
    return (
      <main className="mx-auto max-w-md space-y-4 px-4 pt-8 pb-12">
          ...
          months.map((m) => (
            <div key={m.month} className={`${cardCls} flex items-center justify-between gap-3`}>
              <Link href={`/liff/payslip?m=${m.month}`} className="flex-1">
                <p className="text-sm font-medium text-ink-1">{buildMonthLabel(locale, m.month)}</p>
        ...
        <nav className="flex justify-center gap-4 text-xs">
          <Link href="/liff/summary" className="text-ink-3 hover:text-ink-2">
            {t('backToSummary')}
          </Link>
  ...
  const row = (label: string, value: string, opts?: { strong?: boolean; muted?: boolean }) => (
    <div className={`flex justify-between ${opts?.strong ? 'font-medium' : ''}`}>
      <dt className={opts?.muted ? 'text-ink-4' : 'text-ink-3'}>{label}</dt>
      <dd className={opts?.muted ? 'text-ink-4' : 'text-ink-1'}>{value}</dd>
    </div>
  );
      ...
      {!doc ? (
        ...
        <>
          ...
          <section className={cardCls}>
            <h2 className="text-sm font-semibold text-ink-1">{t('income.title')}</h2>
            <dl className="mt-3 space-y-1.5 text-sm">
              {doc.income.lines.map((l) => (
                <div key={l.key} className="flex justify-between">
                  <dt className="text-ink-3">{lineLabel(l)}</dt>
                  <dd className="text-ink-1">{fmt(l.amount)}</dd>
                </div>
              ))}
              <div className="mt-2 flex justify-between border-t border-line-soft pt-2 font-medium">
                <dt className="text-ink-2">{t('income.total')}</dt>
                <dd className="text-ink-1">{fmt(doc.income.total)}</dd>
              </div>
            </dl>
          </section>
          ...
          {/* Deductions */}
          <section className={cardCls}>
            <h2 className="text-sm font-semibold text-ink-1">{t('deduct.title')}</h2>
            <dl className="mt-3 space-y-1.5 text-sm">
              ...
              <div className="mt-2 flex justify-between border-t border-line-soft pt-2 font-medium">
                <dt className="text-ink-2">{t('deduct.total')}</dt>
                <dd className="text-ink-1">-{fmt(doc.deduct.total)}</dd>
              </div>
            </dl>
          </section>
          ...
          <section className={`${cardCls} bg-primary-50`}>
            <div className="flex items-baseline justify-between">
              <h2 className="text-sm font-semibold text-ink-1">{t('net')}</h2>
              <p className="text-2xl font-bold text-ink-1">{fmt(doc.net)}</p>
            </div>
          </section>
        </>
      )}
