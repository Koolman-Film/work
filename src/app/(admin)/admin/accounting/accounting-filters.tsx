...
export function AccountingFilters({
  ...
  /** Hide the month picker. The chart-of-accounts mapping is a standing
   *  setting, not a monthly one — offering a month there would imply the
   *  mapping could differ per month, which it cannot. */
  monthless?: boolean;
  action?: string;
}) {
    ...
    <form onSubmit={handleSubmit} className="mb-4 flex flex-wrap items-end gap-3">
      ...
      <label className="flex flex-col text-xs text-ink-3">
        ...
        <select
          ...
          className="mt-1 rounded-lg border border-line-strong bg-surface px-3 py-2 text-sm focus:border-primary-400 focus:outline-none focus:ring-2 focus:ring-primary-100"
        >
