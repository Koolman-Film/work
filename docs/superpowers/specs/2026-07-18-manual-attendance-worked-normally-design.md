...
*Trade-off ที่ยอมรับอีกข้อ:* `calc.ts` (`computeLatePenalty`,
`src/lib/payroll/calc.ts:260-287`) เมื่อเปิด `lateThreeStrikeEnabled` จะหักเงิน
เป็น**ก้อนต่อครบทุก N ครั้งมาสาย** (`threeStrikeDays = Math.floor(tier1 /
threeStrikeCount)`) ไม่ใช่หักทีละครั้ง ดังนั้นการยกเว้นแถว `Late` ของครั้งที่
"ทำให้ครบ N" ในเดือนนั้น **ไม่ได้แค่ลดยอดหักไปเสี้ยวหนึ่ง** — มันตัดทั้งก้อนของ
strike นั้นออกทั้งเดือน (เช่น ครบทุก 3 ครั้ง หักครั้งละ 1 วัน: พนักงานมาสายครบ
3 ครั้งพอดีในเดือนนั้น ถ้ายกเว้นครั้งที่ 3 ก็จะไม่โดนหัก 1 วันนั้นเลย ไม่ใช่หัก
บางส่วน) แอดมินที่กดยกเว้นจึงควรรู้ว่าผลกระทบอาจมากกว่าที่ตัวเลข "มาสาย X นาที"
บนแผงเตือนบอกไว้ ทางแก้ (ให้ payroll คิดจากจำนวนครั้งที่เหลือหลังหักยกเว้นแทน)
อยู่นอกขอบเขตงานนี้เพราะต้องแก้ `calc.ts` ซึ่งเป็นโค้ดคำนวณเงินเดือน
